...
class LoginRequiredMiddleware(object):
    ...
    def process_request(self, request):
     ...
     'django.core.context_processors.auth'."
        running_tests = bool(request.environ['SERVER_NAME'] == 'testserver' and request.environ['REMOTE_ADDR'] == '127.0.0.1')
        if not request.user.is_authenticated() and not running_tests:
