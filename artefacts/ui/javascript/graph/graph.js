...
var MARGIN = { top: 5, right: 40, left: 40, bottom: 30  };
var WIDTH = { max: 1000, min: 300};
...
console.log("init height= " + h);
console.log("init width= " + w);
...
function drawGraph(color, id, dataset, goal) {
	...
	addDataPointInteraction(id, data, color);
	// DRAW DATA ANALYSIS
	drawAllDataAnalysis(id, color, data, goal);
...
function eraseGraph(id, data) {
	...
	for(var i = 0; i < points.length; i++) {
		for(var j = 0; j < points[i].length; j++) {
			...
			var pat = "\<br\/\>Your \<b\>" + id +"\<\/b\> was \: \<b\>" + (points[i][j].value).toString() + "\<\/b\>"
...
function addDataPointInteraction(id, data, color) {
	svg.selectAll("." + id + "-point")
            .each(function(d) {
                ...
                d3.select(this)
                	.attr("id", idStr)
                	.attr("title", title + '<a class="pull-right close-button" onclick="$(&quot;#' + 
                						idStr +'&quot;).popover(&quot;hide&quot;);">x</a>');
            });
    var points = checkPointCoord(id);
 	dataPointPopover(id, data, points);       
    $("."+id+"-point").hover(
			function () {
				d3.select(this)
					.attr("opacity", function (d) { 
...
function dataPointPopover(id, data, points) {
	svg.selectAll("." + id + "-point")
            .each(function(d) {
            	var i = data.indexOf(d);
         		var val = d.value;
         		var content = "Your <b>" + id + "</b> was : <b>" + val.toString() + "</b>";
				for(var j = 0; j < points[i].length; j++) {
					if(j == points[i].length - 1)   // <---- only add the content of the data point on top
				...
				var options = {
					...
					'placement': function(context, source){
									...
									// check if x is close to window left limit
